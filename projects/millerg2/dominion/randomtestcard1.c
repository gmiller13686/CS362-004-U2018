...
for(index = 0; index < 2000; index++) {
    ...
    expectedHandcount = state.handCount[currentPlayer];    
    ...
    //Set handPos index in player's hand to the card being played (Village Card is 14)
    state.hand[currentPlayer][handPos] = 14;
    ...
    result = -1;
    pointState = &state;
    result = villageCard(pointState, handPos, currentPlayer);
    ...
    else if(state.numActions != expectedActions) {
        actionError++;
    }
    else {
        successfulTest++;
    ...
    //Print results
    printf("Results for Village card test: \n");
    printf("Number of successful tests: %d\n", successfulTest);
    printf("Number of input errors: %d\n", inputError);
    printf("Number of action errors: %d\n", actionError);
